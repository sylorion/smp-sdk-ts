...
export class Withdrawal {
    ...
    assertIntegerMinorUnits(params) {
        ...
        if (Number.isInteger(amount))
            return;
        throw new Error(`[Withdrawal.${operation}] amount must be an integer in minor units (cents). Received ${amount} (${currency})`);
    }
    /**
     ...
     */
    async submitForApproval(withdrawalRequestId, actorId) {
        const mutation = withdrawalMutations.SUBMIT_FOR_APPROVAL;
        const response = await this.client.mutate(mutation, { withdrawalRequestId, actorId });
     ...
     */
    async getById(withdrawalRequestId) {
        const query = withdrawalQueries.GET_WITHDRAWAL;
        const response = await this.client.query(query, { withdrawalRequestId });
    ...
    async list(filters = {}) {
        ...
        const response = await this.client.query(query, filters);
        return response.withdrawalRequests;
    }
    /**
     * Helper: Lists withdrawals for a specific wallet.
     */
    async listByWallet(walletId, status) {
