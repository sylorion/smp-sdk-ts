...
export function isPeriodicUptake(uptakeForm) {
...
export function stripeRateForCountry(cardCountry) {
 ...
 */
export function calculateStripeFees(amountCents, cardCountry = 'FR') {
...
export function calculatePlatformFee(priceHTCents) {
    return Math.round(priceHTCents * PLATFORM_FEE_RATE);
}
/** Montant de TVA sur un prix HT, en centimes. */
export function calculateVat(priceHTCents, vatPercent) {
    return Math.round(priceHTCents * (vatPercent / 100));
}
function assertValidCents(value, label) {
    if (!Number.isFinite(value) || !Number.isInteger(value) || value < 0) {
        throw new RangeError(`${label} doit être un entier positif en centimes (reçu : ${String(value)})`);
    }
}
/**
...
export function computePricing(input) {
    ...
    const stripeFeeCents = isPeriodic || subtotalCents === 0 ? 0 : calculateStripeFees(subtotalCents, input.cardCountry);
    return {
        priceHTCents,
        vatPercent,
        vatCents,
 ...
 */
export function computeCartPricing(lines, options = {}) {
    const priceHTCents = lines.reduce((sum, line) => sum + Math.round(line.unitPriceHTCents) * (line.quantity ?? 1), 0);
 ...
 * référence.
 */
export function estimateHTFromTotal(totalCents, options = {}) {
